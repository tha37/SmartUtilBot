...
def setup_mbin_handler(app: Client):
    ...
    async def bin_handler(client: Client, message: Message):
        ...
        if message.reply_to_message and message.reply_to_message.document:
            ...
            os.remove(file_name)
            logging.info(f"BINs extracted from the uploaded file by user: {message.from_user.id}")
        else:
            ...
            if len(input_text) < 2:
                ...
                logging.warning(f"Invalid BIN input by user: {message.from_user.id}")
                return
        ...
        if len(bins) > 20:
            ...
            logging.warning(f"User {message.from_user.id} tried to fetch more than 20 BINs")
            return
        ...
        if invalid_bins:
            ...
            logging.warning(f"Invalid BIN formats from user: {message.from_user.id} - {invalid_bins}")
            return
        ...
        async def fetch_bin_info(bin):
            ...
            if isinstance(bin_info, dict):
                if bin_info.get('Status') == "SUCCESS":
                    ...
                    logging.error(f"Error for BIN: {bin} - {error_message}")
                    return f"• **BIN**: `{bin}`\n• **INFO**: {error_message}\n\n"
            else:
                logging.error(f"Invalid bin_info format or response for BIN: {bin} - {bin_info}")
                return f"• **BIN**: `{bin}`\n• **INFO**: Not Found\n\n"
        ...
        logging.info(f"BIN info sent to user: {message.from_user.id}")

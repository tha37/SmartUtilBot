...
async def send_news(client, chat_id, data, country_code, prev_page_token=None, next_page_token=None, message_id=None):
    ...
    if not news_list:
        await client.edit_message_text(chat_id, message_id, "**Google News API Dead**", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if message_id:
        await client.edit_message_text(chat_id, message_id, text, reply_markup=reply_markup, disable_web_page_preview=True, parse_mode=ParseMode.MARKDOWN)
    else:
        await client.send_message(chat_id, text, reply_markup=reply_markup, disable_web_page_preview=True, parse_mode=ParseMode.MARKDOWN)
...
def setup_news_handler(app):
    ...
    async def news(client, message):
        try:
            ...
            if 'temp_message' in locals():
                await client.edit_message_text(message.chat.id, temp_message.id, "**News API Database Error**", parse_mode=ParseMode.MARKDOWN)
            else:
                await client.send_message(message.chat.id, "**News API Database Error**", parse_mode=ParseMode.MARKDOWN)
    ...
    async def prev_page(client, callback_query):
        try:
            ...
            next_page_token = data.get("nextPage")
            # Edit temporary message with news
            await send_news(client, callback_query.message.chat.id, data, country_code, prev_page_token, next_page_token, temp_message.id)
        except ValueError as e:
            logging.error(f"Error fetching news: {e}")
            # Edit temporary message with error
            if 'temp_message' in locals():
                await client.edit_message_text(callback_query.message.chat.id, temp_message.id, "**News API Database Error**", parse_mode=ParseMode.MARKDOWN)
            else:
                await client.send_message(callback_query.message.chat.id, "**News API Database Error**", parse_mode=ParseMode.MARKDOWN)
        except Exception as e:
    ...
    async def next_page(client, callback_query):
        try:
            ...
            # Edit temporary message with news
            await send_news(client, callback_query.message.chat.id, data, country_code, prev_page_token, data.get("nextPage"), temp_message.id)
        except ValueError as e:
            logging.error(f"Error fetching news: {e}")
            # Edit temporary message with error
            if 'temp_message' in locals():
